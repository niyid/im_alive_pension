        ...
        if self.env.user.id and pin:
            ...
            if pensioner_data:
                ...
                if p_first_rec.image:
                    return self.match_face2(p_first_rec.id, face_unknown_base64, context)
                else:
...
#         _logger.info("Image dimension - %s", image.shape)
    ...
    def match_face(self, pensioner_id, face_unknown_base64, context=None):
        _logger.info("Calling match_face - %d and %d", self.env.user.id, pensioner_id)
...
#                 image_file_unknown_brighter.save(file_name_unknown_sharpen, "PNG")
    ...
    @api.model
    def match_face2(self, pensioner_id, face_unknown_base64, context=None):
        _logger.info("Calling match_face2 - %d and %d", self.env.user.id, pensioner_id)
        ...
        if self.env.user.id and face_unknown_base64 and pensioner_id:
            ...
            if len(pensioner_data) == 1:
                for p in pensioner_data:
                    if p.image:
                        face_known_base64 = p.image.decode('base64')
                        
                        if face_known_base64:
                            results = REKOGNITION_CLIENT.compare_faces(SourceImage={'Bytes': face_known_base64}, TargetImage={'Bytes': base64.urlsafe_b64decode(face_unknown_base64)}, SimilarityThreshold=90)
                            _logger.info("results=%s", results)
                            if (len(results['FaceMatches']) > 0 and results['FaceMatches'][0]['Similarity'] >= 95.0):
                                _logger.info("Pensioner Found: %d", pensioner_id)
                                return pensioner_id
                            else:
                                _logger.info("No pensioner Found: %d", pensioner_id)
                                return "Face did not match photo on file. Please try again."
                        else:
                            _logger.info("Pensioner has no photo on file: %d", pensioner_id)
                            return "You require a photo on file and a new photo to match with."
            else:
                return "More than one pensioners found with id " + pensioner_id
        else:
...
#             _logger.info("LFCC DD=%s", lfcc_e_d_dd[0])
    ...
    def activate_payment(self, pensioner_id, payment_code, face_unknown_base64, context=None):
        ...
        pensioner_check_id = self.match_face2(pensioner_id, face_unknown_base64, context)
